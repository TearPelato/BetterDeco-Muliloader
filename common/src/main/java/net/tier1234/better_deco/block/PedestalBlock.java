    ...
    protected ItemInteractionResult useItemOn(ItemStack stack, BlockState state, Level level, BlockPos pos,
        ...
        if(level.getBlockEntity(pos) instanceof PedestalBlockEntity pedestalBlockEntity) {
            ...
            if(!level.isClientSide()) {
                ItemStack itemOnPedestal = pedestalBlockEntity.inventory.getItem(0);

                if(itemOnPedestal.isEmpty() && !stack.isEmpty()) {
                    pedestalBlockEntity.inventory.setItem(0, stack.copyWithCount(1));
                    stack.shrink(1);
                    level.playSound(player, pos, SoundEvents.ITEM_PICKUP, SoundSource.BLOCKS, 1f, 2f);

                } else if(!itemOnPedestal.isEmpty() && stack.isEmpty()) {
                    ItemStack toGive = itemOnPedestal.copy();
                    pedestalBlockEntity.inventory.setItem(0, ItemStack.EMPTY);
